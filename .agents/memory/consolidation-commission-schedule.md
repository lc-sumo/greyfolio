...
For a split structure, the configured upfront share is paid at funding and acts as a credit against total commission earned on funding actually received. Backend commission finalizes only after every effective funding increment clears or when the merchant opts out early. There is no halfway-funded release trigger.
...
**How to apply:** Require the deal-specific breakdown on parent entry, derive increment count from it, and retain cadence, upfront share, expected dates, and confirmed receipt dates. Do not expose LOC draw fields or infer amounts from lender defaults.
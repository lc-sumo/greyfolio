...
1. **Host it.** `docs/DEPLOY.md` covers Replit, Render via `render.yaml`, or any Docker host via `docker-compose.prod.yml` (Postgres + app + a nightly `pg_dump` sidecar, `scripts/backup.sh`). Environment: `DATABASE_URL`, `SESSION_SECRET` (32+ random chars), `BASE_URL`/`APP_ORIGIN` (the public https address), and `SEED=workbook` on the first boot only (then `SEED=none`). Optional: `MAIL_PROVIDER=resend` (or `postmark`) + `MAIL_API_KEY` + a verified `MAIL_FROM` for outgoing email, and `OIDC_*` for SSO.
2. **First sign-in.** Open the portal. While nobody has a password and there is no SSO, the sign-in screen shows *Set up the first admin*: enter the admin email from the roster and choose a password. The screen closes itself once any password exists, so it can never be used to take over a live portal.
3. **Settings › Portal.** Company and portal names (the header, emails and the authenticator label), support email, which emails go out (statements, clawback notices, rep questions, the renewal digest and its hour), the *require two-factor for every admin* switch (turn on your own first), and the dropdown lists (payment frequencies, deal statuses).
4. **Bring the tracker in.** Google Sheets → File → Download → Microsoft Excel (or a CSV of the FUNDED DEALS tab), then Settings › Import from sheet. The preview lists every lender, partner or rep the sheet names that the portal does not know yet, with one-click adds; fix the red rows, import. Re-exporting the whole sheet later is fine: tick *skip rows already in the portal* and only new rows come in. Rows with a Rep Paid Date become paid ledger lines in a run called "Imported from sheet".
5. **Reps.** Settings › Reps: add or deactivate people, set rates, access level and team, set or reset passwords (a readable temporary one is suggested) and reset two-factor if a phone is lost. Reps change their own password and turn on two-factor from the sidebar.
...
8. **Audit log.** Every login, edit, payout, void, password change, settings change, email sent and file uploaded, with the actor's IP, filterable by rep and action, CSV export. CI runs typecheck, tests, both portal builds and the Docker image on every push (`.github/workflows/ci.yml`).

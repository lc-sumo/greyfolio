...
export function adminSettingsRouter(repo: Repo, notify: Omit<NotifyDeps, 'repo'>): Router {
  ...
  r.get('/import-review', async (_req, res) => res.json({ rows: await listTrackerReviews(repo) }));
  ...
  const importOpts = async (req: Parameters<Router>[0]) => {
    const source = await sheetSource(req.body);
    return {
      skipExisting: !!req.body?.skipExisting || !!req.body?.updateExisting,
      updateExisting: !!req.body?.updateExisting,
      ...(source ? { grid: source.grid, sheetName: source.sheetName, matchingSheets: source.matchingSheets } : {}),
    };
  };

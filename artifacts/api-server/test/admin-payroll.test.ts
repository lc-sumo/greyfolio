...
describe('increments paid to reps', () => {
  it('a consolidation pays per lender receipt; the row tracks 4/20 paid', async () => {
    ...
    const created = await admin.post('/api/admin/deals').send({ business: 'Consol Co', fundedDate: today, lender: 'GFE', product: 'CONSOLIDATION - UPFRONT COMM', amount: 100_000, termDays: 200, factor: 1.3, commRate: 10, openerId: 'rep-julian-ribak', openerRate: 35, referralPartner: null, commAmounts: Array(20).fill(5_000) });
    expect(created.status).toBe(201);

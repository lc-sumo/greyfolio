...
describe('auth', () => {
  ...
  it('advertises sign-in methods publicly', async () => {
    const { app } = harness();
    expect((await request(app).get('/auth/methods')).body).toMatchObject({ oidc: false, devAuth: true, password: true, setup: false });
  });
...
describe('admin roster', () => {
  it('reads every rep\'s money from repLedger', async () => {
    ...
    expect(julian).toMatchObject({ name: 'Julian Ribak', team: 'Team Amato', earned: 1_050, paid: 350, held: 250, owed: -250, dealCount: 2 });
  });
});

...
interface AuditEntry { actorRepId: string; actorName: string; action: string; targetRepId: string | null; targetName: string | null; path: string | null; detail?: Record<string, unknown>; at?: string; ip?: string | null }
interface AuditPage { entries: AuditEntry[]; hasMore: boolean; actions: string[] }

const TONE: Record<string, 'teal' | 'amber' | 'red' | 'grey'> = { 'payroll.pay': 'teal', 'deal.create': 'teal', 'login.failed': 'red', 'rep.password': 'amber', 'password.reset': 'amber', 'rep.totp': 'amber', 'settings.update': 'amber', 'view-as': 'grey', 'mail.sent': 'grey' };
...
export function Audit() {
  ...
  const rows = useMemo(() => {
    ...
    return (q.data?.entries ?? []).filter((e) => !s || `${e.actorName} ${e.targetName ?? ''} ${e.action} ${e.ip ?? ''} ${e.path ?? ''} ${JSON.stringify(e.detail ?? {})}`.toLowerCase().includes(s));
  }, [q.data, search]);
    ...
    <Shell eyebrow="Admin" title="Audit log">
      <Card title="Everything the portal recorded" extra={<>newest first · logins, edits, payouts, emails, password changes · showing {q.data?.entries.length ?? 0}{q.data?.hasMore ? <> · <button className="linkish" style={{ color: 'var(--teal)', padding: 0 }} onClick={() => setPages((p) => p + 1)}>load 500 more</button></> : ''}{!DEMO && <> · <a href={csv}>download CSV</a></>}</>}>
        <div className="toolbar" style={{ marginBottom: 12, flexWrap: 'wrap' }}>
          <input className="search" placeholder="Search actor, target, IP, path or detail" value={search} onChange={(e) => setSearch(e.target.value)} style={{ minWidth: 300 }} />
        ...
        {!q.data ? <Loading error={q.error} /> : rows.length === 0 ? <Empty>Nothing recorded{rep || action ? ' for that filter' : ' yet'}.</Empty> : (
          <div className="scroller">
            <div className="table" style={{ ['--cols' as string]: '150px 150px minmax(140px,1fr) 140px 130px minmax(200px,1.1fr) minmax(260px,1.6fr)', minWidth: 1250 }}>
              <div className="tr th"><div className="td">When</div><div className="td">Action</div><div className="td">By</div><div className="td">About</div><div className="td">IP</div><div className="td">Where</div><div className="td">Detail</div></div>
              {rows.map((e, i) => (

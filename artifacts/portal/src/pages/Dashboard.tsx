                  ...
                  <div className="table" style={{ ['--cols' as string]: '90px 80px minmax(170px,1.3fr) 130px 100px 100px minmax(120px,1fr)', minWidth: 800 }}>
                    ...
                    {d.owedToMe.map((v) => (
                      <div className="tr click" key={v.id} onClick={() => setOpen(v.id)}>
                        ...
                        <div className="td muted">{day(v.date)}</div>
                        <div className="td ellipsis" style={{ fontWeight: 600 }}>{v.business}</div>
                        <div className="td ellipsis muted">{v.lender}</div>
                        <div className="td r num">{money(v.share)}</div>
                        <div className="td r num" style={{ fontWeight: 700 }}>{money(v.owed)}</div>

...
import { Card, ClawbackBar, Empty, Loading, Pill, toneFor } from '../components/ui';
import { api, type RepDealView } from '../lib/api';
import { day, money, pct } from '../lib/format';
import { useSession } from '../lib/session';

const COLS = '120px 70px 84px minmax(170px,1.2fr) 130px minmax(150px,1fr) 110px 70px 120px 130px minmax(170px,1fr)';
          ...
          <div className="scroller">
            <div className="table" style={{ ['--cols' as string]: COLS }}>
              <div className="tr th">
                ...
                <div className="td r">Funded</div><div className="td r">Rate</div><div className="td r">My share</div><div className="td">Status</div><div className="td">Clawback</div>
              </div>
              {rows.map((d) => (
                <div className="tr click" key={d.id} onClick={() => setOpen(d.id)}>
                  ...
                  <div className="td r num">{money(d.share)}{(() => { const u = d.lines.find((l) => l.units)?.units; return u ? <div className="subtle" style={{ fontSize: 12.5 }}>Lender {u.collected}/{u.total} · You {u.paid}/{u.total}</div> : null; })()}</div>
                  <div className="td"><Pill tone={toneFor(d.payoutStatus)}>{d.payoutStatus}</Pill></div>
                  <div className="td">
                    {d.clawbackWindow.cleared ? (
                      <span className="cleared" title={d.clawbackWindow.clearsOn ? `Cleared ${d.clawbackWindow.clearsOn}` : d.clawbackWindow.label}><i>✓</i> {d.clawbackWindow.basis === 'none' ? 'No clawback' : 'Cleared'}</span>
                    ) : (
                      <><ClawbackBar fundedDate={d.date} win={d.clawbackWindow} /><div className="subtle num" style={{ fontSize: 12.5, marginTop: 3 }}>{d.clawbackWindow.daysLeft}d left · clears {day(d.clawbackWindow.clearsOn!)}</div></>
                    )}
                  </div>
                </div>
              ))}
              <div className="tr total">
                ...
                <div className="td subtle num">{rows.filter((d) => d.clawbackWindow.cleared).length}/{rows.length} cleared</div>
              </div>
            </div>
          </div>
        )}
